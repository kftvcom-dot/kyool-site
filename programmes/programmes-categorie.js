// PROGRAMMES-CATEGORIE.JS - VERSION CORRIGÉE COMPATIBLE
// Avec hero images dynamiques et loader
...
function getCategorieFromURL() {
  ...
  // Whitelist des catégories autorisées
  ...
  let validatedCat = rawCat;
  if (window.SecurityUtils && typeof window.SecurityUtils.validateCategorySlug === 'function') {
    validatedCat = window.SecurityUtils.validateCategorySlug(rawCat, allowedCategories);
  } else {
    // Validation locale si SecurityUtils n'existe pas
    if (!rawCat || !allowedCategories.includes(rawCat)) {
      validatedCat = null;
    }
  }
...
function loadHeroImage(categorieSlug) {
  ...
  if (config && config.heroImage) {
    ...
    heroImageElement.onerror = function() {
      ...
      this.src = '../media/hero_pour_presentation_prgramme.jpg';
    };
  }
}
...
async function loadCategorie() {
  ...
  if (catSlug === 'documentaire' || catSlug === 'kyool-original' || catSlug === 'tv-shows') {
...
function displayProgrammes() {
  ...
  if (filteredProgrammes.length === 0) {
    ...
    if (noResults) {
      noResults.style.display = 'block';
      noResults.innerHTML = `<p>Aucun programme trouvé pour votre recherche.</p>`;
    }
    return;
  }
  ...
  grid.innerHTML = filteredProgrammes.map(prog => {
    const imagePath = `../media/fiche-programmes/${safeEscape(prog.id)}/image_1500x2100.jpg`;
    
    return `
      <a href="programme-fiche.html?id=${safeEscape(prog.id)}" class="programme-card">
        <div class="programme-image">
          ...
          <div class="programme-overlay">
            <h3 class="programme-title">${safeEscape(prog.titre)}</h3>
...
function updateResultsCount() {
  const resultsCount = document.getElementById('resultsCount');
  if (resultsCount) {
    resultsCount.textContent = filteredProgrammes.length;
  }
}
