// PROGRAMME.JS - VERSION CORRIGÉE COMPATIBLE
...
// Fonction utilitaire locale pour échapper HTML
...
async function loadProgramme() {
  ...
  let programmeId = rawId;
  if (window.SecurityUtils && typeof window.SecurityUtils.validateId === 'function') {
    programmeId = window.SecurityUtils.validateId(rawId);
  }
  
  if (!programmeId) {
    console.error('ID de programme invalide:', rawId);
...
async function loadRecommendationsAsync() {
  ...
  if (container) {
    container.innerHTML = '<p style="text-align: center; color: rgba(255,255,255,.5); grid-column: 1/-1;">🔄 Chargement...</p>';
  }
...
async function loadAllProgrammes() {
  ...
  console.log(`✅ ${allProgrammes.length} programmes chargés`);
}
  ...
  if (youtubeID) {
    document.getElementById('trailerContainer').innerHTML = `
      <div class="trailer-thumbnail" style="position: relative; width: 100%; height: 100%; cursor: pointer;" onclick="openYouTubeModal('${youtubeID}')">
        ...
        <div class="play-button" style="position: absolute; top: 50%; left: 50%; transform: translate(-50%, -50%); width: 80px; height: 80px; background: rgba(255,0,0,0.9); border-radius: 50%; display: flex; align-items: center; justify-content: center;">
...
function displayMotsCles() {
  ...
  container.innerHTML = cles.map(cle => 
    `<span class="mot-cle">${safeEscape(cle)}</span>`
  ).join('');
}
...
function displayRecommendations() {
  ...
  container.innerHTML = recommendations.map(prog => {
    const imagePath = `../media/fiche-programmes/${safeEscape(prog.slug)}/image_1500x2100.jpg`;
    const firstTheme = prog.themes ? safeEscape(prog.themes.split(';')[0].trim()) : '';
    
    return `
      <a href="programme-fiche.html?id=${safeEscape(prog.id)}" class="recommendation-card">
        <div class="recommendation-image">
          <img src="${imagePath}" alt="${safeEscape(prog.titre)}" onerror="this.src='../media/kyool-mascot.png'">
          <div class="recommendation-overlay">
            <h3 class="recommendation-title">${safeEscape(prog.titre)}</h3>
