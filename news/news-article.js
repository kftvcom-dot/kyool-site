// NEWS-ARTICLE.JS - VERSION CORRIGÉE COMPATIBLE
...
async function loadArticle() {
  ...
  let articleId = rawId;
  if (window.SecurityUtils && typeof window.SecurityUtils.validateArticleId === 'function') {
    articleId = window.SecurityUtils.validateArticleId(rawId);
  }
  
  if (!articleId) {
    console.error('ID article invalide:', rawId);
...
function displayArticle() {
  ...
  // ✅ Corps : HTML sécurisé avec fallback
  const articleBody = document.getElementById('articleBody');
  if (articleBody) {
    if (window.SecurityUtils && typeof window.SecurityUtils.setHTMLSafe === 'function') {
      // Utiliser SecurityUtils.setHTMLSafe si disponible
      window.SecurityUtils.setHTMLSafe('articleBody', currentArticle.body || '', true);
    } else if (typeof DOMPurify !== 'undefined') {
      // Fallback 1: Utiliser DOMPurify directement si disponible
      articleBody.innerHTML = DOMPurify.sanitize(currentArticle.body || '');
    } else {
      // Fallback 2: HTML brut (pas idéal mais fonctionne)
      articleBody.innerHTML = currentArticle.body || '';
    }
  }
